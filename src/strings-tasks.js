...
function removeFirstOccurrences(str, value) {
  if (str.includes(value)) {
    ...
    return str.slice(0, startIndex) + str.slice(endIndex, str.length);
  }
  return str;
}
...
function formatTime(minutes, seconds) {
  const minToString = String(minutes);
  const secToString = String(seconds);
  const mm = minToString.padStart(2, '0');
  const ss = secToString.padStart(2, '0');
  return `${mm}:${ss}`;
}
...
function countVowels(str) {
  ...
  for (let i = 0; i <= str.length - 1; i += 1) {
    const subStr = str[i];
    if (vowels.includes(subStr)) result += 1;
  }
  return result;
}
...
function isPalindrome(str) {
  const normalizeStr = str.toLocaleUpperCase().replace(/[^a-zA-Z0-9]/g, '');
  const array = normalizeStr.split('');
  const reverseStr = array.reverse().join('');
